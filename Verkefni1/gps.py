...
class newton:
    ...
    def f1(self, x,y,z,d):
        return pow((x - self.system[0][0]),2) + pow((y - self.system[0][1]),2) + pow((z - self.system[0][2]),2) - pow(c,2)*pow((self.system[0][3]-d),2)

    def f2(self, x, y, z, d):
        return pow((x - self.system[1][0]),2) + pow((y - self.system[1][1]),2) + pow((z - self.system[1][2]),2) - pow(c,2)*pow((self.system[1][3]-d),2)

    def f3(self, x, y, z, d):
        return pow((x - self.system[2][0]),2) + pow((y - self.system[2][1]),2) + pow((z - self.system[2][2]),2) - pow(c,2)*pow((self.system[2][3]-d),2)

    def f4(self,x, y, z, d):
        return pow((x - self.system[3][0]),2) + pow((y - self.system[3][1]),2) + pow((z - self.system[3][2]),2) - pow(c,2)*pow((self.system[3][3]-d),2)


    def dF(self,vigur):
        return np.array([[2*vigur[0]-2*self.system[0][0], 2*vigur[1]-2*self.system[0][1], 2*vigur[2]-2*self.system[0][2], 2*self.system[0][3]*pow(c,2)- 2*pow(c,2)*vigur[3]],
                         [2*vigur[0]-2*self.system[1][0], 2*vigur[1]-2*self.system[1][1], 2*vigur[2]-2*self.system[1][2], 2*self.system[1][3]*pow(c,2)- 2*pow(c,2)*vigur[3]],
                         [2*vigur[0]-2*self.system[2][0], 2*vigur[1]-2*self.system[2][1], 2*vigur[2]-2*self.system[2][2], 2*self.system[2][3]*pow(c,2)- 2*pow(c,2)*vigur[3]],
                         [2*vigur[0]-2*self.system[3][0], 2*vigur[1]-2*self.system[3][1], 2*vigur[2]-2*self.system[3][2], 2*self.system[3][3]*pow(c,2)- 2*pow(c,2)*vigur[3]]])
...
def plot3d(system):
    ...
    plt.show()
def skekkja():
    i1rett = coords(-math.pi/4,math.pi/8)
    i1rettstutt=i1rett[0:3]
    i2rett = coords(math.pi/2,math.pi/6)
    i2rettstutt = i2rett[0:3]
    i3rett = coords(2 * math.pi / 3,3 * math.pi / 8)
    i3rettstutt = i3rett[0:3]
    i4rett = coords(math.pi / 6, math.pi / 4)
    i4rettstutt = i4rett[0:3]
    systemnew = np.array([i1rettstutt,i2rettstutt,i3rettstutt,i4rettstutt])
    n=newton(systemnew)
    vigur = np.array([0, 0, 6370, 0])
    x0 = vigur
    tolerance = 0.01
